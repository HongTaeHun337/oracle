...
/*
    ...
    FORM table-name     -- 데이터소스. 어떤 테이블로부터 데이터를 가져와라
    WHERE search_condition -- 조건 지정 > 원하는 행을 가져온다. 
    GROUP BY group_by_expression -- 그룹을 나눈다.
    HAVING search_condition -- 그룹 조건 지정 > 원하는 그룹만 가져와라
    ORDER BY order_expression [ASC(오름)|DESC(내림)] -- 레코드 정렬 > 정렬 기준(컬럼)
