...
insert into tblUser values ('test', '테스트', '러닝, 메뉴얼독서, 필독서');
...
insert into tblUser values ('cat', '고양이', '운동', '독서', '코딩');
insert into tblUser values ('test', '테스트', '러닝', '메뉴얼독서', null);
...
insert into tblProject project values(1, '홍콩 수출', 1);  --홍길동
insert into tblProject project values(2, 'TV 광고', 2);   --아무개
insert into tblProject project values(3, '매출 분석', 3);  --하하하
insert into tblProject project values(4, '고객 관리', 1);  --홍길동
insert into tblProject project values(5, '대리점 분양', 2); --아무개
...
-- A.3 신규 프로젝트 추가
-- ORA-02291: 무결성 제약조건(HR.SYS_C008482)이 위배되었습니다- 부모 키가 없습니다
...
-- B. '홍길동' 퇴사
-- B.1 '홍길동' 삭제
-- ORA-02292: 무결성 제약조건(HR.SYS_C008482)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
-- 고객 테이블 + 판매 테이블
-- : 어떤 고객(c.name)이 어떤 물건(c.item)을 몇 개를 사갔습니까?
select 
    c.name,
    s.item,
    s.qty
...
--내부 조인
--: 프로젝트 1개 이상 담당하고 있는 직원 정보 + 프로젝트 정보
...
/*
    ...
    4. 셀프 조인, self join
    - 1개의 테이블을 사용하는 조인
    - 테이블이 자기 스스로와 관계를 맺고 있는 경우에만 사용
...
select 
    ...
    department as 부서명,
    boss,
    (select name from tblSelf where seq = s.boss)
from tblSelf s; --직원(자식) 테이블
